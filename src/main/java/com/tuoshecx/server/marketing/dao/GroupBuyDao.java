...
public class GroupBuyDao {
    ...
    private final RowMapper<GroupBuy> mapper = (r, i) -> {
        GroupBuy t = new GroupBuy();
    ...
    @Autowired
    public GroupBuyDao(DataSource dataSource){
        this.jdbcTemplate = new JdbcTemplate(dataSource);
    }

    public void insert(GroupBuy t){
        final String sql = "INSERT INTO marketing_conf_group (id, shop_id, goods_id, name, icon, images, summary, detail, " +
                "help, price, start_time, end_time, is_open, show_order, person, days, update_time, create_time) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        Date now = new Date();
        jdbcTemplate.update(sql, t.getId(), t.getShopId(),t.getGoodsId(), t.getName(), t.getIcon(), DaoUtils.join(t.getImages()),
                StringUtils.defaultString(t.getSummary()), StringUtils.defaultString(t.getDetail()), StringUtils.defaultString(t.getHelp()),
                t.getPerson(), DaoUtils.date(t.getStartTime()), DaoUtils.date(t.getEndTime()), t.getOpen(), t.getShowOrder(), t.getPerson(),
                t.getDays(), DaoUtils.timestamp(now), DaoUtils.timestamp(now));
    }

    public boolean update(GroupBuy t){
        final String sql = "UPDATE marketing_conf_group SET goods_id = ?, name = ?, icon = ?, images = ?, summary = ?, " +
                "detail = ?, help = ?, price = ?, start_time = ?, end_time = ?, is_open = ?, show_order = ?, " +
                "person = ?, days = ? , update_time = ? WHERE id = ?";
        return jdbcTemplate.update(sql, t.getGoodsId(), t.getName(), t.getIcon(), DaoUtils.join(t.getImages()),
                ...
                DaoUtils.date(t.getEndTime()), t.getOpen(), t.getShowOrder(), t.getPerson(), t.getDays(),
                DaoUtils.timestamp(new Date()), t.getId()) > 0;
    }

    public boolean updateOpen(String id, boolean isOpen){
        final String sql = "UPDATE marketing_conf_group SET is_open = ?, update_time = ? WHERE id = ? AND is_delete = false";
        return jdbcTemplate.update(sql, isOpen, DaoUtils.timestamp(new Date()), id) > 0;
    }

    public GroupBuy findOne(String id){
        final String sql = "SELECT * FROM marketing_conf_group WHERE id = ? AND is_delete = false";
        return jdbcTemplate.queryForObject(sql, new Object[]{id}, mapper);
    }

    public boolean delete(String id){
        final String sql= "UPDATE marketing_conf_group SET is_delete = ?, update_time = ? WHERE id = ? AND is_delete = false";
    ...
    public long count(String shopId, String name, Boolean isOpen){
        ...
        sql.append("SELECT COUNT(id) FROM marketing_conf_group");
    ...
    public List<GroupBuy> find(String shopId, String name, Boolean isOpen, int offset, int limit){
        StringBuilder sql = new StringBuilder(200);
        sql.append("SELECT * FROM marketing_conf_group ");
        buildWhere(sql, shopId, name, isOpen);
